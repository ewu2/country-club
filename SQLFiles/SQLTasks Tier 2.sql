...
SELECT DISTINCT f.name AS court, m.firstname AS member
FROM `Members` AS m
...
AND (CASE WHEN b.memid = 0 AND (guestcost*slots) THEN (guestcost*slots)
...
SELECT b.facid, f.name, COUNT(facid) AS member_use
FROM `Bookings` AS b
LEFT JOIN `Facilities` AS f
USING (facid)
WHERE memid > 0
...
SELECT b.facid, f.name, strftime('%m', starttime) AS month, COUNT(facid) AS member_use
FROM `Bookings` AS b
LEFT JOIN `Facilities` AS f
USING (facid)
WHERE memid > 0
